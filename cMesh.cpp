...
// init_terrain() - 
//////////////////////////////////////////////////////////////////////////////////////////////////////////
void cMesh::gen_terrain()
{
	float start = -10.0f;
	float end   = 10.0f;
	float step = (end-start) / (m_v_count-1);

	for (int j = 0; j < m_v_count; j++)
	{
		float x = j * step;
		float i = int(x); // integer
		float f = x - i; // fraction

		float u = f * f * (3.0 - 2.0 * f); // custom cubic curve
		float y = mix(random(i), random(i + 1.0), u); // using it in the interpolation

		m_vList[j].m_v.x = start + j * step;
		m_vList[j].m_v.y = y;
		m_vList[j].m_v.z = 0.0f;
	}
}

void cMesh::createTerrainVBO()
{
	// code used to create a buffer needed to render this object

	glBufferData(GL_ARRAY_BUFFER, sizeof(cVertex) * 1 * m_v_count, m_vList, GL_STATIC_DRAW); // points
...
void cMesh::render(cScene * pScene, cShaderInfo* pShaderInfo)
{
	...
	if (strcmp(m_name, "ground") == 0)
	{
		...
		glDrawArrays(GL_LINE_STRIP, 0, m_v_count);
		return;
	}
